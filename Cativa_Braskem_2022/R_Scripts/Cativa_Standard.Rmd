...
```{r, fig.width = 15, fig.height = 5}
Tabela_Teste <- data.frame("Mes" = rep(x =  seq(from = 1, to = 12, by = 1), times = nrow(x = Tabela_Excesso_Horizon)),
                           "Valor" = 0,
                           "Ano" = 0)
...
for(i in 1:nrow(x = Tabela_Excesso_Horizon)){
  Tabela_Teste$Valor[Contador:(Contador + 11)] <- Tabela_Excesso_Horizon[i,]
  Tabela_Teste$Ano[Contador:(Contador + 11)] <- rep(x = rownames(x = Tabela_Excesso_Horizon)[i], times = 12)
  Contador <- Contador + 12
}

#Por Algum Fator Que Ainda Não Compreendi, Os Valores Estavam Como Lista
Tabela_Teste$Valor <- unlist(x = Tabela_Teste$Valor)

ggplot(data = Tabela_Teste, aes(x = Mes, y = Valor)) + geom_line(aes(colour = Ano), size = 1.2) + theme_clean(base_size = 16) +
...
for(i in 1:nrow(x = Tabela_Excesso_Horizon)){
  Tabela_Teste$Valor[Contador:(Contador + 11)] <- Tabela_Fluxo_Acumu[i,]
  Tabela_Teste$Ano[Contador:(Contador + 11)] <- rep(x = rownames(x = Tabela_Fluxo_Acumu)[i], times = 12)
  Contador <- Contador + 12
}
